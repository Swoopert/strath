      ...
      const CLIENT_ID = '876064215712-ddtaql16676las9kav9ic25otlg22844.apps.googleusercontent.com';
      const API_KEY = 'AIzaSyAawJODlUPO69vIgDVc9OyvAkcs1owekrA';
	...
	await loadDates(soloAvailability,tandemAvailability);
      ...
      async function getSoloEvents() {
        ...
        try {
          const request = {
            'calendarId': '9a68ac4da3c337495101cad469722d969f389952bfc498362ee058a4a508a518@group.calendar.google.com',
            'singleEvents': true,
            'orderBy': 'startTime',
            'q': 'Solo Course Spaces Available: '
          };
        ...
        for (i=0;i<events.length;i++) {
          summaryArr = events[i].summary.split(" ");
          if (summaryArr[4] != "0") {
      ...
      async function getTandemEvents() {
        ...
        try {
          const request = {
            ...
            'singleEvents': true,
            'orderBy': 'startTime',
            'q': 'Tandem Skydive Spaces Available: '
          };
          response = await gapi.client.calendar.events.list(request);
        } catch (err) {
          return err.message;
...
Epoch.prototype.calConfig = function () //PRIVATE: initialize calendar variables
{
	...
	this.displayYearInitial = this.firstDate.getFullYear(); //the initial year to display on load
	this.displayMonthInitial = this.firstDate.getMonth(); //the initial month to display on load (0-11)
	this.rangeYearLower = this.firstDate.getFullYear();
	this.rangeYearUpper = this.curDate.getFullYear()+1;
	this.minDate = this.firstDate;
	this.maxDate = this.curDate.getFullYear()+1;
...
CalCell.prototype.setClass = function ()  //private: sets the CSS class of the cell based on the specified criteria
{
	...
	else if(this.owner.systemAvailability.indexOf(this.date.getFullYear()+'-'+(this.date.getMonth()+1)+'-'+this.date.getDate()) > -1) {
